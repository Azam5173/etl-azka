...
from etl.transformers.etl_transformer import EtlETL, EtlSourceConfig, EtlTargetConfig

class TestEtlETLMethods(unittest.TestCase):
    """
    Testing the EtlETL class.
    """
    ...
    def setUp(self):
        ...
        conf_dict_src = {
            'src_first_extract_date': '2022-10-01',
        ...
        conf_dict_trg = {
            ...
            'trg_key': 'report1/etl_daily_report1_',
        ...
        data = [['AT0000A0E9W5', 'SANT', '2022-10-15', '12:00', 20.19, 18.45, 18.20, 20.33, 877],
                ['AT0000A0E9W5', 'SANT', '2022-10-16', '15:00', 18.27, 21.19, 18.27, 21.34, 987],
                ['AT0000A0E9W5', 'SANT', '2022-10-17', '13:00', 20.21, 18.27, 18.21, 20.42, 633],
                ['AT0000A0E9W5', 'SANT', '2022-10-17', '14:00', 18.27, 21.19, 18.27, 21.34, 455],
                ['AT0000A0E9W5', 'SANT', '2022-10-18', '07:00', 20.58, 19.27, 18.89, 20.58, 9066],
                ['AT0000A0E9W5', 'SANT', '2022-10-18', '08:00', 19.27, 21.14, 19.27, 21.14, 1220],
                ['AT0000A0E9W5', 'SANT', '2022-10-19', '07:00', 23.58, 23.58, 23.58, 23.58, 1035],
                ['AT0000A0E9W5', 'SANT', '2022-10-19', '08:00', 23.58, 24.22, 23.31, 24.34, 1028],
                ['AT0000A0E9W5', 'SANT', '2022-10-19', '09:00', 24.22, 22.21, 22.21, 25.01, 1523]]
        self.df_src = pd.DataFrame(data, columns=columns_src)
        self.s3_bucket_src.write_df_to_s3(self.df_src.loc[0:0],
        '2022-10-15/2022-10-15_BINS_XETR12.csv','csv')
        self.s3_bucket_src.write_df_to_s3(self.df_src.loc[1:1],
        '2022-10-16/2022-10-16_BINS_XETR15.csv','csv')
        self.s3_bucket_src.write_df_to_s3(self.df_src.loc[2:2],
        '2022-10-17/2022-10-17_BINS_XETR13.csv','csv')
        self.s3_bucket_src.write_df_to_s3(self.df_src.loc[3:3],
        '2022-10-17/2022-10-17_BINS_XETR14.csv','csv')
        self.s3_bucket_src.write_df_to_s3(self.df_src.loc[4:4],
        '2022-10-18/2022-10-18_BINS_XETR07.csv','csv')
        self.s3_bucket_src.write_df_to_s3(self.df_src.loc[5:5],
        '2022-10-18/2022-10-18_BINS_XETR08.csv','csv')
        self.s3_bucket_src.write_df_to_s3(self.df_src.loc[6:6],
        '2022-10-19/2022-10-19_BINS_XETR07.csv','csv')
        self.s3_bucket_src.write_df_to_s3(self.df_src.loc[7:7],
        '2022-10-19/2022-10-19_BINS_XETR08.csv','csv')
        self.s3_bucket_src.write_df_to_s3(self.df_src.loc[8:8],
        '2022-10-19/2022-10-19_BINS_XETR09.csv','csv')
        ...
        data_report = [['AT0000A0E9W5', '2022-10-17', 20.21, 18.27, 18.21, 21.34, 1088, 10.62],
                       ['AT0000A0E9W5', '2022-10-18', 20.58, 19.27, 18.89, 21.14, 10286, 1.83],
                       ['AT0000A0E9W5', '2022-10-19', 23.58, 24.22, 22.21, 25.01, 3586, 14.58]]
    ...
    def test_extract_no_files(self):
        ...
        return_value=[extract_date, extract_date_list]):
            etl_etl = EtlETL(self.s3_bucket_src, self.s3_bucket_trg,
                         self.meta_key, self.source_config, self.target_config)
            df_return = etl_etl.extract()
    ...
    def test_transform_report1_emptydf(self):
        ...
        return_value=[extract_date, extract_date_list]):
            ...
            with self.assertLogs() as logm:
                df_result = etl_etl.transform_report1(df_input)
    ...
    def test_transform_report1_ok(self):
        ...
        """
        # Expected results
        log1_exp = 'Applying transformations to Etl source data for report 1 started...'
        log2_exp = 'Applying transformations to Etl source data finished...'
    ...
    def test_load(self):
        ...
        """
        # Expected results
        log1_exp = 'Etl target data successfully written.'
        log2_exp = 'Etl meta file successfully updated.'
        df_exp = self.df_report
        meta_exp = ['2022-10-17', '2022-10-18', '2022-10-19']
        # Test init
        extract_date = '2022-10-17'
        extract_date_list = ['2022-10-16', '2022-10-17', '2022-10-18', '2022-10-19']
        ...
        return_value=[extract_date, extract_date_list]):
            etl_etl = EtlETL(self.s3_bucket_src, self.s3_bucket_trg,
                         self.meta_key, self.source_config, self.target_config)
            with self.assertLogs() as logm:
                etl_etl.load(df_input)
                # Log test after method execution
                self.assertIn(log1_exp, logm.output[1])
                self.assertIn(log2_exp, logm.output[4])
        # Test after method execution
        trg_file = self.s3_bucket_trg.list_files_prefix(self.target_config.trg_key)[0]
        data = self.trg_bucket.Object(key=trg_file).get().get('Body').read()
        out_buffer = BytesIO(data)
        df_result = pd.read_parquet(out_buffer)
        self.assertTrue(df_exp.equals(df_result))
        meta_file = self.s3_bucket_trg.list_files_prefix(self.meta_key)[0]
