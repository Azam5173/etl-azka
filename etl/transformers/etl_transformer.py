...
class EtlETL():
    """
    Reads the ETL data, transforms and writes the transformed to target
    """

    def __init__(self, s3_bucket_src: S3BucketConnector,
                 s3_bucket_trg: S3BucketConnector, meta_key: str,
                 src_args: EtlSourceConfig, trg_args: EtlTargetConfig):
        """
        ...
        self.extract_date = '2022-10-20'
        self.extract_date_list = '2022-10-30'
        self.meta_update_list = '2022-11-30'
    ...
    def extract(self):
        ...
        """
        self._logger.info('Extracting etl source files started...')
        files = [key for date in self.extract_date_list\
                     for key in self.s3_bucket_src.list_files_prefix(date)]
        if not files:
            data_frame = pd.DataFrame()
        else:
            data_frame = pd.concat([self.s3_bucket_src.read_csv_to_df(file)\
                for file in files], ignore_index=True)
        self._logger.info('Extracting etl source files finished.')
        return data_frame
    ...
    def transform_report1(self, data_frame: pd.DataFrame):
        ...
        if data_frame.empty:
            ...
            return data_frame
        self._logger.info('Applying transformations to etl source data for report 1 started...')
        ...
        self._logger.info('Applying transformations to etl source data finished...')
        return data_frame
    ...
    def load(self, data_frame: pd.DataFrame):
        ...
        self._logger.info('etl target data successfully written.')
        # Updating meta file
        MetaProcess.update_meta_file(self.meta_update_list, self.meta_key, self.s3_bucket_trg)
        self._logger.info('etl meta file successfully updated.')
        return True
